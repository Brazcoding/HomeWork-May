...
const Numbers = persons.map(function(persons){
    return persons.phone;
});

console.log(Numbers);

// create and print a list of website and email pairs (pair them in a string, array, or object)
const Sauce = persons.map(function(person){
    return `"${person.website} , ${person.email}"`;
});
...
const user = greek.filter((greek) => greek.userId = 9);
console.log(user);

  const homeworkFilter = (greek, keys, con) =>
   greek.filter(con).map(el => 
    keys.reduce((acc, key) => {
         acc[key] = el[key]; 
         return acc;
    }, {})
);
 console.log(homeworkFilter(greek, ['userId', 'completed'], task => task.completed === false));
...
const deus = [
 ...
 const user = deus.filter((deus) => deus.userId = 5);
 console.log(user);

 //NEXT QUESTION

// //  JS Classes & Constructors Practice
// // In a .js file, create a few classes. Create one "general category" class,
...
class general extends student {
    constructor(history, math, science, studyHall) {
        super(history, math, science);
    this.studyHall = studyHall;
  }
  QuePasa() {
    return `Hello! I teach ${this.history}, and I assistant teach ${this.gym} I also help student with ${studyHall}`;
